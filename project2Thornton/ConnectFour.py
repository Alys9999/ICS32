...
def _require_valid_column_count(columns: int) -> None:
    ...
    if columns < 4 or columns > MAX_COLUMNS:
        raise ValueError((f'columns must be an int between 4 and {MAX_COLUMNS}'))


def _require_valid_row_count(rows: int) -> None:
    '''Raises a ValueError if the given number of rows is invalid.'''
    if rows < 4 or rows > MAX_ROWS:
        raise ValueError(f'rows must be an int between 4 and {MAX_ROWS}')